...
class StubContext : public ToolContext {
public:
...
ToolExecutionRecord
ToolExecutor::invoke_one(const ToolCall& call, std::shared_ptr<ToolContext> ctx) {
...
ToolExecutor::execute(const std::vector<ToolCall>& calls) {
    ...
    auto ctx = std::make_shared<StubContext>();
    for (const auto& group : groups) {
        std::vector<std::future<ToolExecutionRecord>> futures;
        for (auto idx : group) {
            futures.push_back(std::async(std::launch::async, [this, &calls, ctx, idx]() {
